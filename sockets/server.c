...
main ()
{
  ...
  do
  {
      ...
      { 
        ...
        {
            //in child
            requesthandler (sockfd);
        }
        else if (pid > 0)
        {
            //in parent
            /*
            here the parent should save address and pid of the child server in a struct array or whatever
            */
        }
    }
}
...
killallprocess (char *process)
{
  ...
  if (prc != NULL)
  {
      if (prc->isrunning != 0)
      {
          ...
          write (sockfd, "All instances have been killed\n",
           sizeof ("All instances have been killed\n"));
      }
      else
      {
          write (sockfd, "All instances have already been killed\n",
           sizeof ("All instances have already been killed\n"));
      }
  }
  else
...
struct processinfo *
searchpname (char *name, struct processinfo *ptr)
{
